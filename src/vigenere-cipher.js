  ...
  specSymbol = [" ", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "!", "@", "#", "$", "(", ")", ",", ".", "/", "|", "*", "-", "&", "^", "%", "'", ":", "_", "+", "="];
    ...
    encrypt(message, key) {
      ...
      point: for (let i = 0; i < arrMessage.length; i++) {
        for (let y = 0; y < this.specSymbol.length; y++) {
          if (arrMessage[i] === this.specSymbol[y]) {
      ...
      for (let y = 0; y < arrMessage.length; y++) {
        if (arrMessage.length !== arrKey.length) {
          arrKey.push(arrKey[y]);
        } else {
          break;
        }
      }
      ...
      for (let i = 0; i < key.length; i++) {
        ...
        if ((messageI + keyI) >= 26) {
          result += this.alphabet[(messageI + keyI) - 26];
        } else {
          result += this.alphabet[messageI + keyI];
        }
      }
      ...
      for (let i = 0; i < arrMessage.length; i++) {
        for (let y = 0; y < this.specSymbol.length; y++) {
          if (arrMessage[i] === this.specSymbol[y]) {
            result.splice(i, 0, this.specSymbol[y]);
          }
        }
      }
    ...
    decrypt(message, key) {
      ...
      for (let i = 0; i < key.length; i++) {
        ...
        if ((messageI + 26 - keyI) > 26) {
          result += this.alphabet[(messageI + 26 - keyI) - 26];
        } else {
          result += this.alphabet[messageI + 26 - keyI];
        }
      }
      result = result.split('');
      arrMessage = linkMessage.split('');
